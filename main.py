...
import winsound  # Impor library winsound untuk suara beep di Windows

# Memuat model terlatih
model_path = "9v-m/weights/best.pt"
model = YOLO(model_path)

# Membuka file video (bisa statis maupun realtime)
video_path = "video/test.mp4"
cap = cv2.VideoCapture(video_path) # untuk realtime  cap = cv2.VideoCapture(0) dan jika ingin menggunakan camera external ubah cap = cv2.VideoCapture(1)
